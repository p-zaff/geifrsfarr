...
```mermaid
graph TD
A[Cargo Vago] --> B{É Cargo Imutável?}
B -->|SIM| C[Há substituto preparado?]
B -->|NÃO| D[Movimentação interna?]
C -->|SIM| E[Posse imediata do substituto]
C -->|NÃO| F[Reunião Extraordinária em 15 dias]
D -->|SIM| G[Votação DE - maioria simples]
D -->|NÃO| H[Inscrição aberta para associados]
F --> I[Eleição entre membros DE]
H --> G
```
